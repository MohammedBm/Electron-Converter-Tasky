...
// DONE: Communicate to MainWindow process that videos
...
// DONE: Communicate to MainWindow that the user wants
...
export const convertVideos = () => (dispatch, getState) => {
  const video = _.map(getState().videos);
  ipcRenderer.send('conversion:start', video);
  ipcRenderer.on('conversion:end', (event, {video, outputPath}) => {
    dispatch({ type: VIDEO_COMPLETE, payload: video })
  })  
};

// DONE : Open the folder that the newly created video
// exists in
